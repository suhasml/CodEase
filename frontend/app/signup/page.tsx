...
import { getAuth, createUserWithEmailAndPassword, updateProfile, GoogleAuthProvider, GithubAuthProvider, TwitterAuthProvider, signInWithPopup, AuthProvider, onIdTokenChanged } from 'firebase/auth';
...
const SignUpForm = () => {
    ...
    const handlePostAuthRedirect = async (idToken: string, isNewUser: boolean) => {
        try {
            ...
            // Check if the redirect is for subscription/checkout
            if (redirectPath === 'checkout' || redirectPath === 'subscription') {
                if (selectedPlanId) {
                    router.push('/checkout');
                } else {
                    router.push('/subscription');
            ...
            if (pendingCheckout === 'true' && selectedPlanId) {
                if (isClient) sessionStorage.removeItem('pendingCheckout');
                router.push('/checkout');
            } else if (redirectPath === 'checkout' || redirectPath === 'subscription') {
                router.push('/subscription');
            } else {
                const sessionId = generateUUID();
                router.push(`/chat/${sessionId}`);
    ...
    const handleSignUp = async () => {
        ...
        try {
            ...
            const userResponse = await createUserInDb(user.uid, email, idToken);
            ...
            // Store user data and ID token in cookies using cookie-utils
            setCookie('user', {
                uid: user.uid,
                email: user.email,
                displayName: name,
            ...
            // Handle redirection based on credits and context
            // Email signup is always a new user, so pass true
            await handlePostAuthRedirect(idToken, true);
        } catch (err: any) {
    ...
    const handleProviderSignUp = async (provider: AuthProvider): Promise<void> => {
        setIsLoading(true);
        try {
            ...
            // Save user data and ID token in cookies using cookie-utils
            setCookie('user', {
                uid: user.uid,
                email: user.email,
            ...
            // Handle redirection based on credits and context
            // Check if this is a new user based on the createUserInDb response
            const isNewUser = userResponse.is_new === true;
            await handlePostAuthRedirect(idToken, isNewUser);
        } catch (error) {
        ...
        if (screenWidth < 640) {
            return (
                <div className="flex flex-col space-y-3 mb-6">
                    ...
                    <button
                        onClick={() => handleProviderSignUp(new TwitterAuthProvider())}
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 bg-gray-900/60 backdrop-blur-md p-5 sm:p-8 rounded-xl shadow-2xl w-full max-w-md border border-gray-700/50 overflow-hidden group hover:border-blue-500/30 transition-all duration-500">
                ...
                <h2 className="text-xl sm:text-2xl font-bold text-center mb-2 bg-gradient-to-r from-white to-blue-200 bg-clip-text text-transparent">Create Account</h2>
                <p className="text-center text-gray-400 mb-5 sm:mb-6 text-sm">
                    Sign up for a CodEase account
                ...
                {/* Social Sign-up Buttons - Responsive */}
                {renderSocialButtons()}
